/*
 * Written by Donald Chinn Sept 19, 2003
 * Modified by Justin Arnett Dec 6, 2015
 */
...
public class BinaryHeap {
    ...
    public static BinaryHeap buildHeap(Comparable[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            ((PrimHeapNode) newHeap.elements[i+1]).setHeapPosition(i+1);
        }
    ...
    /**
     * Checks if the new value at the given index violates binary heap
     * properties. If it does, percolates as needed.
     * @param index    the index being verified in the heap array.
     * @return the new index of the data.
     */
    public int updateHeapPosition(int index) {
    	return percolateUp(index);
    }
    ...
    private int percolateUp(int index) {
        ...
        while (index > 1) {
            if (temp.compareTo(elements[index/2]) < 0) {
                ...
                ((PrimHeapNode) elements[index]).setHeapPosition(index);
                index = index / 2;
            } else {
                break;
            }
